...
from pd.internal.assets.asset_registry import (
    ObjAssets, DataVehicle, PairVehicleColor,
    DataWheel, PairVehicleWheel, UtilAssetCategories, database
)
...
def init_asset_registry_version(ig_version: str):
    ...
    try:
        next(iv for iv in IgVersion.list() if iv.name == ig_version)
    except StopIteration:
        raise PdError(
            f"Couldn't fetch Asset Registry for Ig version '{ig_version}'. "
            f"This Ig version is not available."
        )
    registry_content = fetch_ig_asset_registry(ig_version)
...
def get_vehicle_wheel_poses(vehicle_name: str, wheel_name: str) -> List[Pose6D]:
    ...
    (
        wheel_offset_front_left_s,
        wheel_offset_front_right_s,
        wheel_offset_back_left_s,
        wheel_offset_back_right_s
    ) = (
    ...
    if not wheel_offset_front_left_s or not wheel_offset_front_right_s or \
            not wheel_offset_back_left_s or not wheel_offset_back_right_s:
        return []
