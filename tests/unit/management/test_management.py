...
from pd.management import Ig, IgQuality, IgStatus, IgVersion, LabelEngineVersion, Levelpak, LevelpakVersion, SimVersion
    ...
    def test_create_with_levelpaks(self, **kwargs):
        ...
        m.post(
            ...
            json={
                ...
                "ig_url": None,
                "created_at": "2022-04-08T07:36:04+05:00",
                "levelpak": {"A2_Kerrytown": "v1", "SC_Highlands": "v3"},
            },
        )
        ig = Ig.create(levelpaks={"A2_Kerrytown": "v1", "SC_Highlands": "v3"})
        assert m.last_request
        data = m.last_request.json()
        assert data["levelpak"] == {"A2_Kerrytown": "v1", "SC_Highlands": "v3"}
        assert ig
            ...
            json={
                "igs": [
                    ...
                    },
                    {
                        ...
                        "created_at": "2022-04-08T07:36:04+05:00",
                        "shutdown_time": None,
                        "levelpak": {"SF_6thAndMission_medium": "v1"},
                    },
                ]
            },
        )
...
class TestIgVersion:
    def test_list(self, requests_mock):
        ...
        requests_mock.get(
            f"{_BASE_URL}/test/ig_versions", json=[{"name": "v1.2.3"}, {"name": "another"}, {"name": "default"}]
        )
...
class TestLevelpak:
    def test_list(self, requests_mock):
        ...
        requests_mock.get(
            f"{_BASE_URL}/test/levelpaks",
            json=[
                {"default_version": "v1", "name": "SC_W8thAndOrchard", "versions": ["v1"]},
                {"default_version": "v2", "name": "SJ_KettmanAndOrinda_aus", "versions": ["v1", "v2"]},
                {"default_version": "v3", "name": "SJ_EssexAndBradford", "versions": ["v1", "v2", "v3"]},
            ],
        )
...
class TestLevelpakVersions:
    def test_list(self, requests_mock):
        ...
        uuid_1, uuid_2, uuid_3 = [str(uuid4()) for _ in range(3)]
        requests_mock.get(
            f"{_BASE_URL}/test/levelpaks/versions",
            json=[
                {
...
class TestSimVersion:
    def test_list(self, requests_mock):
        ...
        requests_mock.get(
            f"{_BASE_URL}/test/sim_versions", json=[{"name": "v1.2.3"}, {"name": "another"}, {"name": "default"}]
        )
...
class TestLabelEngineVersion:
    def test_list(self, requests_mock):
        ...
        requests_mock.get(
            f"{_BASE_URL}/test/le_versions", json=[{"name": "v1.2.3"}, {"name": "another"}, {"name": "default"}]
        )
