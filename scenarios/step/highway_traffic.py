...
import pd.state
import pd.umd
from pd.assets import get_vehicle_wheel_names, get_vehicle_wheel_poses, get_wheel_radius
...
def generate_route(umd_map, start_lane_segment_id: int) -> List[Tuple[float, float, float]]:
    """
    ...
    def generate_vehicle_agent(self) -> pd.state.VehicleAgent:
        if self.vehicle_type and self.wheel_poses:
            rotation = pd.state.Pose6D.from_euler_angles(
                ...
                y_metres=0.0,
                z_metres=0.0,
                alpha_radians=-self.sim_agent.wheel_angle_radians,
                beta_radians=0.0,
                gamma_radians=0.0,
            )
            wheel_poses_rotated = [p * rotation for p in self.wheel_poses]
        else:
...
def cli(preview, sensor, output_dir, force, debug, step_options: StepScriptContext = None):
    ...
    with session:
        ...
        while time_sec < MAX_SIM_TIME_SEC:
            ...
            state = pd.state.State(simulation_time_sec=time_sec, world_info=world_info, agents=agents)
            ...
            if do_capture_frame:
                ...
                for sensor in sensor_list:
                    ...
                    save_image(rgb_output_path, sensor.name, frame_count, cv2.cvtColor(rgb_image, cv2.COLOR_RGB2BGR))
                    preview_rgb_image = (
                    ...
                    if sensor.capture_depth:
                        try:
                            sensor_data = session.query_sensor_data(
                                agent_ego.id, sensor.name, pd.state.SensorBuffer.DEPTH
                            )
                    ...
                    if sensor.capture_segmentation:
                        try:
                            sensor_data = session.query_sensor_data(
                                agent_ego.id, sensor.name, pd.state.SensorBuffer.SEGMENTATION
                            )
                    ...
                    if sensor.capture_instances:
                        try:
                            sensor_data = session.query_sensor_data(
                                agent_ego.id, sensor.name, pd.state.SensorBuffer.INSTANCES
                            )
